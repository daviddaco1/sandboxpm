...
| `packages/resolver` | Semver resolution, dependency tree, `sandboxpm.lock`, typosquat/low-trust risk scan | `Resolver` (`resolve`, `resolveFromLock`), `checkPackageRisk`, `typosquatMatch` |
| `packages/linker` | Non-flat `node_modules` from store hard links + symlinks | `Linker` (`link`, `unlink`) |
| `packages/scripts` | Interactive script approval + Docker/Hyper-V sandbox runner + package-risk prompt | `ScriptPrompt`, `SandboxRunner`, `PackageRiskPrompt` |
| `packages/cli` | `bin.ts`: commander.js entry point, orchestrates the above | `install`, `add`, `remove`, `init`, `audit`, `whitelist`, `trust`, `block`, `cache`, `config`, `ls`, `why`, `outdated`, `info`, `search`, `run`/`exec`, `update`, `version`, `link`/`unlink`, `pack`/`publish`, `login`/`logout` |
...
1. **Resolver**: BFS over the dependency graph, first-satisfying-version-wins per name (pnpm-style dedup); peer dep failures warn and skip rather than throwing. Writes `sandboxpm.lock` (JSON, sorted keys, atomic write). Every name visited is also run through `checkPackageRisk` (`packages/resolver/src/risk.ts`): a Damerau-Levenshtein edit-distance check against a bundled snapshot of popular package names (`packages/resolver/data/popular-packages.json`), escalated to `severity: 'high'` when the matched name is also new (<90 days old / few published versions) or has ≤1 maintainer. Results land in `tree.riskFindings` — always empty for `resolveFromLock` (no registry access, nothing to check).
2. **Package trust/block check** (`packages/cli/src/bin.ts`'s `install()`, between Resolve and Fetch): any resolved package name in `.sandboxpmrc`'s `blockedPackages` aborts immediately, before any tarball is downloaded — this applies to lockfile-based installs too, since it's a pure name comparison. Non-empty `riskFindings` are then handed to `PackageRiskPrompt` (`packages/scripts/src/risk-prompt.ts`), gated by `policies.onPackageRisk` (`continue`/`prompt`/`abort`); a `trust` decision persists to `trustedPackages`, a `block` decision persists to `blockedPackages` and aborts. Each finding's outcome is written to `~/.sandboxpm/reports/risk-{name}-{timestamp}.json`, visible via `sandboxpm audit` alongside script-run reports.
3. **Fetcher**: streams each tarball while hashing, and verifies the hash against the registry's `dist.integrity` **before** extracting — never extract-then-verify. Skips re-downloading if a cached per-tarball manifest shows every file hash is already in the store. Concurrency-limited via `p-limit`.
4. **Store**: `put()` copies into the CAS via a temp-file + atomic rename; `link()` hard-links into place and falls back to `copyFile` on `EXDEV`/`EPERM` (cross-volume stores, OneDrive, or Windows without Developer Mode).
5. **Linker**: builds `node_modules/.sandboxpm/{name}@{version}/node_modules/{name}` from store hard links, then symlinks (junctions on Windows) direct deps into the root `node_modules` and transitive deps into each package's own scope. Falls back to `.cmd` shims when Windows file-symlink creation hits `EPERM`.
6. **ScriptPrompt / SandboxRunner**: install scripts are partitioned by `.sandboxpmrc` policy into whitelist/blacklist/prompt/abort; interactive decisions can persist back to `.sandboxpmrc`. Approved scripts run via `dockerode` in a read-only, `CapDrop: ALL`, seccomp-restricted container on an isolated bridge network; on Windows Docker daemons it instead uses `Isolation: hyperv` (no seccomp/CapDrop there). A native (unsandboxed) fallback exists for sandbox-start failures or Linux-ELF/Windows-host native-addon mismatches, but requires an explicit double-confirmation.

Known gaps in the package-risk check (by design, not oversight): no live CVE/advisory feed, no live npm download-count signal, no re-check on lockfile-based installs, and only a small fixed ASCII homoglyph map (not full Unicode confusables). The bundled popular-package list is a manually curated snapshot, refreshed by hand.

### Config resolution

`loadRc` walks up from the project directory looking for `.sandboxpmrc`, validates enum fields (`sandbox.networkMode`, `policies.onWarn`, `policies.onBlock`, `policies.onPackageRisk`), and deep-merges onto defaults — it never throws for a missing file. This is separate from the global JSON config at `~/.sandboxpm/config.json` (`storeDir`, `cacheDir`, `reportsDir`), written via temp-file + rename. See `.sandboxpmrc.example` at the repo root for the full schema (sandbox resource limits, policy actions, registries, whitelist/blacklist, `trustedPackages`/`blockedPackages`, env passthrough, CAS gc settings). Note `whitelist`/`blacklist` govern SCRIPT execution for a package sandboxpm has already decided to install; `trustedPackages`/`blockedPackages` govern whether the package itself is flagged/installed at all — deliberately separate concepts.
